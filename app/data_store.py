...
import cross_docks_polling as cd_polling

utc_zone = tz.tzutc()
to_zone = tz.gettz('Australia/Melbourne')

stock_file_path = "/Wholesale/APIs (Anna's Dad)/Uphance/DataStore/data_stock.csv"
orders_file_path = "/Wholesale/APIs (Anna's Dad)/Uphance/DataStore/data_orders.csv"

def get_uphance_stock_levels(customer):
	global stock_file_path
	...
	while page :
	    ...
	    if response[0]:
		    ...
		    for p in data['products']:
		        row_dict = {}
		        row_dict['Date'] = [datetime.now().replace(tzinfo=utc_zone).astimezone(to_zone).replace(tzinfo=None)]
		        row_dict['p_id'] = [p['id']]
...
def or_file_sent(customer,file_data):
	...
	for i in len(eans):
		row_dict = {}
		row_dict['date_ordered'] = [datetime.now().replace(tzinfo=utc_zone).astimezone(to_zone).replace(tzinfo=None)]
